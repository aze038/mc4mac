...
final class FakeGmailMailbox: @unchecked Sendable {
    ...
    /// The addresses the account sends from, as users.settings.sendAs.list answers them, with
    /// any field Gmail keeps, such as each address's signature.
    var sendAs: [[String: Any]] {
        get { lock.withLock { state.sendAs } }
        set { lock.withLock { state.sendAs = newValue } }
    }
