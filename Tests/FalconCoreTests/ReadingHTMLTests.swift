...
final class ReadingHTMLTests: XCTestCase {
    ...
    /// A card of the conversation stack leaves less room above and below the message than the
    /// reading pane, and its message is otherwise prepared as the pane's is.
    func testACardOfTheConversationStackLeavesLessRoomAroundTheMessage() {
        let pane = ReadingHTML.page(body: outlook, parts: [], allowRemote: false, dark: true, ownColours: false, installedFamilies: [])
        let card = ReadingHTML.page(body: outlook, parts: [], allowRemote: false, dark: true, ownColours: false, inStack: true,
                                    installedFamilies: [])
        XCTAssertTrue(pane.contains("padding:12px 29px 30px 29px;"), pane)
        XCTAssertTrue(card.contains("padding:6px 29px 10px 29px;"), card)
        XCTAssertEqual(card.replacingOccurrences(of: "6px 29px 10px 29px", with: "12px 29px 30px 29px"), pane)
    }
