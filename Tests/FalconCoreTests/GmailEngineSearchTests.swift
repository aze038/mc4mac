...
final class GmailEngineSearchTests: XCTestCase {
        ...
        func run(_ steps: [SearchTypingPlan.Step]) async {
            for step in steps {
                ...
                case .lookUpIDs(let query):
                    lookups += 1
                    await search?.end()
                    let next = GmailSearchSession(query: query, source: source, store: store, transport: transport)
                    search = next
                    await next.lookUp()
                case .fetchRows:
    ...
    func testTheRowsOfASearchAreTheIndexsOwnSoEveryActionWorksOnThem() async throws {
        let (transport, store, source) = try await mailbox()
        let search = GmailSearchSession(query: "invoice", source: source, store: store, transport: transport)
        await search.lookUp()
