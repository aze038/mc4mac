...
public enum ReadingHTML {
    ...
    /// text, `plain` then being true.
    public static func page(body: String, plain: Bool = false, parts: [MIMEAttachment], allowRemote: Bool, dark: Bool,
                            ownColours: Bool, installedFamilies: Set<String> = ReadingHTML.installedFamilies()) -> String {
        let csp = allowRemote
        ...
        let style = "<style>:root{color-scheme:light;} html,body{background:#ffffff;margin:0;}\(inversion) "
            ...
            + "padding:12px 29px 30px 29px;overflow-wrap:break-word;} p{margin:0 0 16px;} pre{white-space:pre-wrap;font-family:inherit;} "
