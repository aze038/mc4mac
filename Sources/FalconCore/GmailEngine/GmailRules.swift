...
extension GmailActions {
    ...
    public func handleArrivals(_ arrivals: [GmailArrival]) async -> GmailArrivalOutcome {
        ...
        // The engine has already judged by Gmail's clock; this also judges by the Mac's, as for
        // arrivals made here.
        let eligible = arrivals.filter { arrival in
            arrival.runsRules && arrival.labels.contains(.inbox) && !arrival.labels.contains(.spam) && !arrival.labels.contains(.trash)
