...
struct ComposeView: View {
    ...
    /// keeps nothing. One closed or discarded before they arrive is left as it went: putting
    /// them in then would bring it back.
    private func fetchPictures() {
        ...
        Task { @MainActor in
            ...
            guard !fetched.isEmpty, draft != nil, model.drafts[id] != nil else { return }
            ...
            if untouched { draft?.keepUntouched() }
            commitDraft()
        }
    }
