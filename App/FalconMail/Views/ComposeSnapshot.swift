...
/// `-FalconMailSnapshotOnly list` it draws the message list alone; with `-FalconMailSnapshotOnly
/// signature-import` it draws only the Signatures pane offering an import, the import sheets for
/// Outlook and Gmail, macOS's refusal, and the pane after an import, all with made-up signatures.
...
enum ComposeSnapshot {
    ...
    @MainActor static func runIfRequested() {
        ...
        let model = AppModel()
        if UserDefaults.standard.string(forKey: "FalconMailSnapshotOnly") == "signature-import" {
