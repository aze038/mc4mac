...
struct SignaturesSettings: View {
    ...
    var body: some View {
        SettingsScroll {
            Text("Edit signature:").font(.system(size: 13, weight: .semibold))

            HStack(alignment: .top, spacing: 16) {
                VStack(spacing: 0) {
                    Table(model.accounts, selection: $selected) {
                        TableColumn("Signature name") { account in
                            Text(account.signature.trimmed.isEmpty ? "None" : signatureName(account))
                        }
                    }
                    .frame(height: 180)
                    HStack(spacing: 0) {
                        Button { } label: { Image(systemName: "plus").frame(width: 26, height: 20) }
                            .disabled(true).help("Each account keeps one signature")
                        Divider().frame(height: 14)
                        Button { clear() } label: { Image(systemName: "minus").frame(width: 26, height: 20) }
                            .disabled(account == nil || account?.signature.isEmpty == true)
                        Spacer()
                        Button("Edit") { editing = true }.disabled(account == nil)
                    }
                ...
                VStack(spacing: 0) {
                    ...
                    ScrollView {
                        Text(account?.signature.isEmpty == false ? "-- \n\(account!.signature)" : "No signature yet")
                            .font(.system(size: 13))
                            .frame(maxWidth: .infinity, alignment: .leading)
                            .padding(10)
                    }
            ...
            Text("Choose default signature:").font(.system(size: 13, weight: .semibold))
            SettingsRow(label: "Account:", labelWidth: 140) {
                Picker("", selection: Binding(get: { selected ?? model.accounts.first?.id }, set: { selected = $0 })) {
                    ForEach(model.accounts) { a in
                        Text(a.displayName.isEmpty ? a.email : "\(a.displayName) (\(a.email))").tag(Optional(a.id))
                    }
                }
                .labelsHidden().frame(width: 420)
            }
            SettingsRow(label: "New messages:", labelWidth: 140) {
                Text(account?.signature.isEmpty == false ? signatureName(account!) : "None").font(.system(size: 13))
            }
            SettingsRow(label: "Replies/Forwards:", labelWidth: 140) {
                Text(account?.signature.isEmpty == false ? signatureName(account!) : "None").font(.system(size: 13))
            }
        }
        .sheet(isPresented: $editing) {
            if let account {
                SignatureEditor(account: account).environment(model)
            }
        }
        .onAppear { if selected == nil { selected = model.accounts.first?.id } }
    }

    private func signatureName(_ account: AccountInfo) -> String {
        account.displayName.isEmpty ? "Main" : "Main"
    }
...
struct SignatureEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Signature for \(account.email)").font(.title3.bold())
            TextEditor(text: $text)
                .font(.system(size: 13))
                .frame(width: 480, height: 220)
                .overlay(RoundedRectangle(cornerRadius: 6).stroke(Color.primary.opacity(0.2)))
            Text("FalconMail puts a line of two dashes above the signature so other mail apps can hide it in replies.")
                .font(.caption).foregroundStyle(.secondary)
            HStack {
        ...
        .padding(20)
        .onAppear { text = account.signature }
    }
}
